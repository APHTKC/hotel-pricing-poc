...
def breakfast_included(features: list[str]) -> bool | None:
    ...
    if any(term in joined for term in ("不含早餐", "不含早", "無早餐", "純住宿", "breakfast not included", "room only")):
        return False
    if any(term in joined for term in ("含早餐", "含早", "早餐專案", "餐點: 早餐", "breakfast included", "with breakfast")):
        return True
    return None
